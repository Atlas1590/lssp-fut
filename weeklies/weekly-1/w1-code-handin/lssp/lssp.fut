...
let redOp (pred2 : i32 -> i32 -> bool)
  ...
  let newlss = if connect==true then max((lcsx + lisy), max(lssx,lssy)) else max(lssx, lssy)
  let newlis = if lisx==tlx && connect==true then lisx+lisy else lisx
  let newlcs = if lcsy==tly && connect==true then lcsx+lcsy else lcsy
  let newtl  = tlx+tly
...
--mangler newlss i linjen efter last, og 1 i32 på mapop result
